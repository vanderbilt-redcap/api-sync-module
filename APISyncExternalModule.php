...
class APISyncExternalModule extends \ExternalModules\AbstractExternalModule
{
	...
	public function renderExportNowHtml() {
		$exportNow = $this->getProjectSetting('export-now');
		$exportProgress = $this->getProjectSetting('export-progress');
		$exporting = (!$this->isExportCancelled() && ($exportNow || $exportProgress !== null));

		if ($exporting) {
			$html = "<td><button class='api-sync-export-all-button' disabled>Export All Records</button></td><td>An export is already in progress. Let the current export finish or cancel it manually before attempting to export all records.</td>";
		} else {
			$html = "<td><button class='api-sync-export-all-button'>Export All Records</button></td><td>Exports all existing records now.  No records will be removed.</td>";
		}
		return $html;
	}
	...
	public function renderSyncNowHtml() {
		...
		if ($currentSyncMessage) {
			?>
			<p>
				<tr><td colspan=2><?=$currentSyncMessage?><a href="<?=$this->getUrl('cancel-sync.php');?>">Click here</a> to cancel it.</td></tr>
			</p>
			<?php
		} else {
			...
			?>
                <tr>
                    <td><form action="<?=$syncNowUrl?>" method="post">
                        <button>Import Now</button>
                    </form></td>
                    <td>Imports from all sources now.</td>
                </tr>
            <tr class="retry" style="display: none">
                <td><form action="<?=$syncNowUrl?>" method="post">
                    <button>Retry Failed Import</button>
                    <input type="hidden" name="retry-log-id">
                </form></td>
                <td>Continues the last failed import where it left off.</td>
            </tr>
			<?php
		}
	}
